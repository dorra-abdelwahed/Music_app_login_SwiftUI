...
class SignUpViewModel: ObservableObject{
    ...
    @Published var checkNewsletter = false
    ...
    var readyToSubmit: AnyPublisher<Bool, Never> {
        ...
        return Publishers.CombineLatest3(validatedUsername, validatedPassword, $checkBoxSelected)
            .map { name, pass, check in
                
                return name == .valid && pass == .valid && check
            }
            .eraseToAnyPublisher()
    }
